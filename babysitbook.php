...
if ($row['maid_id'] != $hlp && $row['firstname'] != $fname){
	...
	header("Location: babysitting.php");
		...
		if($row['maid_id'] = $hlp && $row['firstname'] = $fname && $row['availability'] == 'available'){
			...
			$my_sqli="INSERT INTO `babybook`(`House-help_ID`, `firstname`, `start_date`, `duration`, `end_date`, `baby`) VALUES ('$hlp','$fname','$start','$duration','$end','$baby')";
